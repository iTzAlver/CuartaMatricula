        ...
        for i in range(rows):
            for j in range(columns):
                ...
                if fijado != "" and indf < len(fijado):
                    if ((j + 1 >= int(iniciof)) and (str(i) not in lineanofijada)) and (
                        ((j in contp) or (j in conts)) or not args.diagonales
                    ):
                        if fijado[indf] != "^":
                            a += fijado[indf]
                            notf = True
                ...
                if not notf:
                    if (
                        j + 1 >= int(inicioint) and j + 1 <= int(finalint)
                    ) or args.t == False:
...
parser.add_argument(
    ...
    nargs=1,
    dest="ejecuta",
    help="Crea un subproceso del ejecutable con el que se quieren probar los casos",
)
parser.add_argument(
    "-fno",
    type=str,
    nargs=2,
    dest="ejecuta_casos_externos",
    help="Crea un subproceso del ejecutable ,pero se alimenta con casos externos",
)
parser.add_argument(
    "-fij",
    type=str,
    nargs="*",
    dest="fijado",
    help="Fijado de caracteres en algunas posiciones",
)
parser.add_argument(
    "-diag",
    type=str,
    nargs="*",
    dest="diagonales",
    help="Fijado de caracteres en diagonales",
)
parser.add_argument(
    "-no",
    type=str,
...
parser.add_argument(
    ...
    nargs="*",
    dest="intercalado",
    help="Elige que posiciones son caracteres o numeros",
)
...
parser.add_argument(
    "--invn",
    help="Invierte el orden de los mensajes nº ronda nº columna",
    action="store_true",
)
parser.add_argument("-n", type=str, nargs=1, dest="size", help="Numero de lineas")
parser.add_argument(
    "-nchar", type=int, nargs=1, dest="nchar", help="Numero de caracteres por columna"
)
...
if args.ejecuta_casos_externos:
    ...
    if args.ejecuta:
        ruta = args.ejecuta[0]
        p = subprocess.run(
            [os.getcwd() + "\\" + ruta], text=True, input=texto_a_ejecutar
        )
